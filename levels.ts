...
export const levels: Level[] = [
  { title: 'Level 1: Big Bear', desc: 'One large friendly bear with soft rounded shapes.', shapes: [
    ...
    sh('body','ellipse','Bear body',{cx:200,cy:236,rx:92,ry:106}), sh('head','circle','Bear head',{cx:200,cy:132,r:78}),
    sh('earL','circle','Left ear',{cx:140,cy:76,r:32}), sh('earR','circle','Right ear',{cx:260,cy:76,r:32}),
    sh('earInL','circle','Left inner ear',{cx:140,cy:78,r:16},'#fff'), sh('earInR','circle','Right inner ear',{cx:260,cy:78,r:16},'#fff'),
    sh('snout','ellipse','Snout',{cx:200,cy:150,rx:38,ry:30},'#fff'), sh('nose','ellipse','Nose',{cx:200,cy:139,rx:12,ry:9},'#111'),
    sh('smile','path','Smile',{d:'M184 158 C193 170 207 170 216 158'},'#111','stroke',2),
    sh('eyeL','circle','Left eye',{cx:170,cy:122,r:7},'#111'), sh('eyeR','circle','Right eye',{cx:230,cy:122,r:7},'#111'),
    sh('belly','ellipse','Belly',{cx:200,cy:250,rx:54,ry:66},'#fff'),
    sh('armL','ellipse','Left arm',{cx:116,cy:228,rx:34,ry:64,transform:'rotate(22 116 228)'}), sh('armR','ellipse','Right arm',{cx:284,cy:228,rx:34,ry:64,transform:'rotate(-22 284 228)'}),
    sh('footL','ellipse','Left foot',{cx:148,cy:334,rx:42,ry:28}), sh('footR','ellipse','Right foot',{cx:252,cy:334,rx:42,ry:28})
  ]},
  { title: 'Level 2: Two Robots Talking', desc: 'Two friendly robots facing each other.', shapes: [
    ...
    sh('lBody','rect','Left robot body',{x:52,y:166,width:112,height:126,rx:20}), sh('lHead','rect','Left robot head',{x:62,y:82,width:92,height:74,rx:18}),
    sh('lAnt','path','Left antenna',{d:'M108 82 L108 46'},'#111','stroke',2), sh('lBall','circle','Left antenna ball',{cx:108,cy:38,r:10}),
    sh('lEye1','circle','Left robot eye',{cx:88,cy:116,r:7},'#111'), sh('lEye2','circle','Left robot eye',{cx:128,cy:116,r:7},'#111'),
    sh('lSmile','path','Left robot smile',{d:'M88 138 C100 148 118 148 130 138'},'#111','stroke',2), sh('lScreen','rect','Left screen',{x:82,y:194,width:52,height:38,rx:8},'#fff'),
    sh('lArm','path','Left arm',{d:'M54 196 C28 210 28 250 56 262'},'#eee','stroke',9), sh('lWheel','ellipse','Left wheel base',{cx:108,cy:314,rx:50,ry:18}),
    sh('rBody','rect','Right robot body',{x:236,y:166,width:112,height:126,rx:20}), sh('rHead','rect','Right robot head',{x:246,y:82,width:92,height:74,rx:18}),
    sh('rAnt','path','Right antenna',{d:'M292 82 L292 46'},'#111','stroke',2), sh('rBall','circle','Right antenna ball',{cx:292,cy:38,r:10}),
    sh('rEye1','circle','Right robot eye',{cx:272,cy:116,r:7},'#111'), sh('rEye2','circle','Right robot eye',{cx:312,cy:116,r:7},'#111'),
    sh('rSmile','path','Right robot smile',{d:'M272 138 C284 148 302 148 314 138'},'#111','stroke',2), sh('rScreen','rect','Right screen',{x:266,y:194,width:52,height:38,rx:8},'#fff'),
    sh('rArm','path','Right arm',{d:'M346 196 C372 210 372 250 344 262'},'#eee','stroke',9), sh('rWheel','ellipse','Right wheel base',{cx:292,cy:314,rx:50,ry:18})
  ]},
  { title: 'Level 3: House and Tree', desc: 'A storybook house, tree, sky, and cloud.', shapes: [
    sh('sky','rect','Sky',{x:0,y:0,width:400,height:250},'#f7f7f7'), sh('grass','rect','Ground',{x:0,y:250,width:400,height:150}),
    ...
    sh('sun','circle','Sun',{cx:330,cy:62,r:34},'#fff'), sh('trunk','path','Tree trunk',{d:'M70 344 V238 C70 224 94 224 94 238 V344 Z'}),
    ...
    sh('wall','rect','House wall',{x:160,y:196,width:150,height:118,rx:8}), sh('roof','path','Roof',{d:'M140 198 L236 112 L330 198 Z'}),
    sh('chimney','rect','Chimney',{x:278,y:124,width:28,height:52,rx:4}), sh('door','path','Door',{d:'M214 314 V260 C214 236 254 236 254 260 V314 Z'},'#fff'),
    sh('winL','rect','Left window',{x:174,y:220,width:36,height:34,rx:6},'#fff'), sh('winR','rect','Right window',{x:260,y:220,width:36,height:34,rx:6},'#fff'),
  ...
  { title: 'Level 5: Big Whale', desc: 'A friendly whale with calm underwater shapes.', shapes: [
    ...
    sh('weedL','path','Left seaweed',{d:'M72 354 C48 322 96 292 76 252 M96 354 C74 318 126 290 104 250'},'#eee','stroke',3), sh('weedR','path','Right seaweed',{d:'M324 354 C300 322 348 292 328 252 M350 354 C328 318 378 290 356 250'},'#eee','stroke',3)
  ]}
];
